...
4.  **Set the data directory**
    ...
    The app reads experiment files from a folder set by the environment variable `EXCEL_DIR`. Either modify the path in `global.R` (line 70) or add the path variable to a `.Renviron` file in the project root. The file `Renviron_example.txt` shows an example of how the `.Renviron` file should look like. 
